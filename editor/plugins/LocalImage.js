...
var LocalImage = dojo.declare("mils.editor.plugins.LocalImage", LinkDialog.ImgLinkDialog, {
	// summary:
	...
	_initialFileUploader: function(){
		...
		if(_this.uploadable && !_this._fileUploader){
			...
			_this.connect(fup, "onComplete", function(data){
                var key = 0;
                for (key in data[0].resources) break;
				urlInput.set("value", data[0].resources[key]); //Single selection
